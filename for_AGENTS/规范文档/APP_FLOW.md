...
**流程步骤:**
1. 用户访问站点根路径 `/`，默认进入 Landing 页面
2. 用户从 Landing 进入 `/home` 或直接搜索公司
3. 输入股票代码或搜索公司
4. 跳转至 `/earnings/[symbol]`
5. 页面并行获取数据:
   - 公司基本信息 (`companies`)
   - 财报数据 (`earnings`)
   - AI 分析结果 (`ai_analyses`)
...
### 2.1 首页性能渲染流程 (2026-02-27)

1. 用户进入 `/`（Landing）
2. 用户进入 `/home`
3. 服务端读取缓存首页数据 (`unstable_cache`, `revalidate=300`)
4. 客户端先渲染首屏核心块（Hero + 核心标题）
5. 非首屏模块通过可见区触发挂载（`ViewportRender`）
6. 列表模块采用“首批渲染 + 加载更多”
7. 前端上报 Web Vitals 到 `/api/web-vitals` 用于后续回归
...
1. 用户访问 `/assistant` 页面
2. 页面加载时获取动态建议:
   - GET `/api/assistant/chat?action=suggestions&symbol=AAPL`
   - 显示快捷问题按钮
3. 用户输入问题或点击快捷问题
4. POST `/api/assistant/chat` 发送消息
5. 后端处理流程:
   - 将用户问题转为向量 (Cohere Embedding API)
   - 向量相似度搜索 (`search_documents` RPC)
   - 提取 Top-5 相关财报内容
   - 构建 RAG Prompt (上下文 + 对话历史 + 用户问题)
   - 调用 DeepSeek API 生成回答
   - 保存对话到 `chat_messages` 表
6. 流式返回 AI 回答
7. 前端实时显示打字效果
...
**流程步骤:**
1. 用户访问 `/portfolio` 页面
2. GET `/api/portfolio` 获取持仓数据
3. 后端检查股价数据新鲜度:
   - 检查 `last_updated` 时间戳
   - 如果超过 15 分钟，触发更新
4. 并行获取最新股价 (FMP API):
   - 批量请求所有持仓股票
   - 更新 `companies` 表的 `current_price`
5. 计算实时盈亏:
   - 市值 = 持仓数量 × 最新价格
   - 盈亏 = 市值 - 成本
   - 盈亏率 = 盈亏 / 成本 × 100%
...
Cohere API (生成查询向量)
    ↓
PGVector 相似度搜索 (document_embeddings)
    ↓
提取相关上下文 (Top-5)
    ↓
构建 RAG Prompt (上下文 + 历史 + 问题)
    ↓
DeepSeek API (生成回答)
    ↓
保存对话 (chat_messages)
