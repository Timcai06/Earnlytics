...
export default function Header({ onMenuToggle, isMenuOpen }: HeaderProps) {
  ...
  const showAuthenticated = hydrated && !loading && Boolean(user);
  const showGuest = hydrated && !loading && !user;
      ...
      <div className="flex h-full items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          {showAuthenticated ? (
            ...
            <div
              aria-hidden="true"
              className="h-9 w-20 rounded-full border border-border bg-surface/70"
            />
          )}
