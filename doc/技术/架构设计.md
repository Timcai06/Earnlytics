...
| 研报数据 | Choice/Wind API | 券商研报、盈利预测 | ¥500-8000/月 |
| AI分析 | DeepSeek | 多维度投资决策分析 | ¥20-40/月 |
| PDF解析 | Marker/Unstructured | 研报PDF文本提取 | 免费 |
| 向量检索 | Supabase pgvector | 研报语义检索 | 免费 |
...
数据源采集:
┌──────────┐  ┌──────────────┐  ┌─────────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│FMP API   │  │Yahoo Finance │  │FRED API     │  │NewsAPI   │  │券商API   │  │SEC EDGAR │
│(财报)    │  │(估值指标)    │  │(宏观数据)   │  │(新闻)    │  │(研报)    │  │(详细数据)│
└────┬─────┘  └──────┬───────┘  └──────┬──────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘
     │               │                │              │             │             │
     └───────────────┴────────────────┴──────────────┴─────────────┴─────────────┘
                                     │
                         ...
                         │  ├─ industry_benchmarks │
                         │  ├─ peer_comparison     │
                         │  ├─ research_reports    │ ← 新增：研报数据
                         │  ├─ analyst_tracking    │ ← 新增：分析师追踪
                         │  ├─ earnings_consensus  │ ← 新增：盈利预测
                         │  └─ macro_policies      │
                         └───────────┬─────────────┘
                                     │
         ┌───────────────────────────┼───────────────────────────┐
...
┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
│ 五维基本面分析   │      │ 估值分析         │      │ 研报聚合分析     │
│ (Financials)    │      │ (Valuation)     │      │ (Research)      │
├─────────────────┤      ├─────────────────┤      ├─────────────────┤
│ • ROE杜邦分析   │      │ • P/E历史区间   │      │ • 分析师评级    │
│ • ROIC评估      │      │ • 行业对比      │      │ • 目标价共识    │
│ • 现金流健康    │      │ • DCF模型       │      │ • 研报摘要      │
│ • 成长性评估    │      │ • 估值分位数    │      │ • 观点分歧度    │
└────────┬────────┘      └────────┬────────┘      └────────┬────────┘
